...
class _RasterizeGaussians(torch.autograd.Function):
        ...
        if raster_settings.debug:
            ...
            try:
                if raster_settings.spherical:
                    num_rendered, color, radii, geomBuffer, binningBuffer, imgBuffer, depth, opacity, n_touched = _C.rasterize_spherical_gaussians(*args)
                else:
    ...
    def backward(ctx, grad_out_color, grad_out_radii, grad_out_depth, grad_out_opacity, grad_n_touched):
        ...
        if raster_settings.debug:
            ...
            try:
                if raster_settings.spherical:
                    grad_means2D, grad_colors_precomp, grad_opacities, grad_means3D, grad_cov3Ds_precomp, grad_sh, grad_scales, grad_rotations, grad_tau = _C.rasterize_spherical_gaussians_backward(*args)
                else:
